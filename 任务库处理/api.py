...
class BaseClient:
    """客户端基类，封装网络通信和日志"""
    def __init__(self, source_name: str = "APP", log_enabled: bool = True,
    ...
    def log(self, text: str, level: str = "INFO"):
        ...
        with open(f"{log_dir}/{time.strftime('%Y-%m-%d')}.log", "a", encoding='utf-8') as f:
            f.write(log_line + "\n")
...
class TaskQueueClient(BaseClient):
    ...
    def __init__(self, to_name: str, message: str, first: int = 1,
                 from_name: Optional[str] = None, **kwargs):
        super().__init__(**kwargs)
        # 添加任务，保存返回的ID
        self.id = self._send_request(
            qp="add",
            name=from_name or self.source_name,
            to_name=to_name,
            message=message,
            first=str(first)
        )
    ...
    def get(self, from_name: Optional[str] = None) -> Any:
        """获取任务结果，返回服务器响应的原始数据"""
    ...
    def delete(self, from_name: Optional[str] = None) -> bool:
        """删除任务，返回布尔值"""
    ...
    def stop(self, from_name: Optional[str] = None) -> bool:
        """停止服务器"""
        result = self._send_request(
            qp="stop",
            name=from_name or self.source_name
...
class RunAPI(BaseClient):
    ...
    def run(self, find_name: Optional[str] = None, from_name: Optional[str] = None) -> Union[Dict, List, None]:
        """
        获取下一个任务。
        如果 find_name 为 "all"，返回所有任务列表。
        否则返回单个任务字典，并自动将任务 ID 保存到 self.current_task。
        """
        kwargs = {"name": from_name or self.source_name}
    ...
    def return_(self, message: str, from_name: Optional[str] = None, task_id: Optional[str] = None) -> bool:
        """
    ...
    def delete(self, from_name: Optional[str] = None, task_id: Optional[str] = None) -> bool:
        """删除任务，参数同 return_"""
        tid = task_id or (self.current_task and self.current_task.get("id"))
        if not tid:
