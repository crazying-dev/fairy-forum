...
class web:
	def __init__(self):
		...
		log(f"start TCP | listen ({ip if ip else '0.0.0.0'}||{addr}||{backlog}) | run __web")
		self.run = Run()
		self.main_loop()
	...
	def main_loop(self):
		...
		while True:
			try:
				...
				log(f"TCP | call {client_addr} True ")
	...
	def handle_client(self, client_socket: socket.socket, client_addr: tuple):
		...
		try:
			...
			if not len_data:
				log(f"TCP | Client {client_addr} disconnected without sending data")
				return
			data_len = int.from_bytes(len_data, byteorder='big')
			recv_data = b''
			while len(recv_data) < data_len:
				chunk = client_socket.recv(min(data_len - len(recv_data), 1024))
				if not chunk:
					log(f"TCP | Client {client_addr} disconnected midway")
					return
				recv_data += chunk
			recv_msg = recv_data.decode('utf-8')
			log(f"TCP | Client {client_addr} received {recv_msg}")

			# 2. 解析并处理请求
			response_data = self.process_request(recv_msg)
			...
			log(f"TCP | Error processing request from client {client_addr} | error {e}")
		finally:
			client_socket.close()
			log(f"TCP | Connection with client {client_addr} closed")

	def process_request(self, recv_msg: str) -> Union[None, str, bool]:
